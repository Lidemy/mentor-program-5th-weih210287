...
/* harmony export */   "addCommentsAPI": () => (/* binding */ addCommentsAPI)
/* harmony export */ });
...
function addCommentsAPI(baseURL, myData, cb) {
  jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
    type: 'POST',
...
function loadComment({ content, nickname, site_key, create_at }, isPrepend) {
  const template = `
    <div class="card">
  ...
  `
  isPrepend ? jquery__WEBPACK_IMPORTED_MODULE_0___default()(`.${site_key}-cards`).prepend(template) : jquery__WEBPACK_IMPORTED_MODULE_0___default()(`.${site_key}-cards`).append(template)
}
...
function init(options) {
  ...
  const { formTemplate, commentSubmitSelector, nicknameSelector, contentSelector, siteKeySelector, cardsSelector, btnLoadSelector } = (0,_template__WEBPACK_IMPORTED_MODULE_1__.getForm)(siteKey)
  const containerElement = jquery__WEBPACK_IMPORTED_MODULE_0___default()(containerSelector)
  containerElement.append(formTemplate)

  // css
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('<style></style>', {
    type: 'text/css'
  }).appendTo('head')
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('style').append(_template__WEBPACK_IMPORTED_MODULE_1__.cssTemplate)

  // 讀取 comments
  getComments()

  // 新增 comment（做兩件事：1. POST 2. 動態新增 comment）
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(commentSubmitSelector).submit((e) => {
    e.preventDefault()
    ...
    const myData = {
      ...
      site_key: jquery__WEBPACK_IMPORTED_MODULE_0___default()(siteKeySelector).val()
    }

    // POST api
    ;(0,_api__WEBPACK_IMPORTED_MODULE_2__.addCommentsAPI)(BASE_URL, myData, (data) => {
      if (!data.ok) {
      ...
      for (const comment of allComments) {
        ;(0,_utils__WEBPACK_IMPORTED_MODULE_3__.loadComment)(comment, true)
      }
      jquery__WEBPACK_IMPORTED_MODULE_0___default()(`${cardsSelector} .card`).length > 5 ? jquery__WEBPACK_IMPORTED_MODULE_0___default()(`${cardsSelector} .card:last-child`).remove() : ''
    })
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(btnLoadSelector).click((e) => {
    console.log('click')
    getComments()
  })
  ...
  function getComments() {
    if (isEnd) return
    ;(0,_api__WEBPACK_IMPORTED_MODULE_2__.getCommentsAPI)(BASE_URL, siteKey, before, (data) => {
      if (!data.ok) {
        return alert(data.message)
      }
      ...
      const allComments = data.comments
      for (const comment of allComments) {
        ;(0,_utils__WEBPACK_IMPORTED_MODULE_3__.loadComment)(comment)
      }

      const { length } = allComments
      if (length < 5) {
        isEnd = true
        jquery__WEBPACK_IMPORTED_MODULE_0___default()(btnLoadSelector).hide()
      } else {
